...
// Функции для работы с учебным годом
function getStudyYear($date = null) {
    $current_date = $date ? new DateTime($date) : new DateTime();
    $current_year = (int)$current_date->format('Y');
    $current_month = (int)$current_date->format('m');
    
    // Если текущий месяц с сентября по декабрь - учебный год текущий-следующий
    if ($current_month >= 9) {
        return $current_year . '-' . ($current_year + 1);
    } else {
        // Если с января по август - учебный год предыдущий-текущий
        return ($current_year - 1) . '-' . $current_year;
    }
}

function getStudyYearStart($study_year) {
    list($start_year, $end_year) = explode('-', $study_year);
    return new DateTime($start_year . '-09-01');
}

function getStudyYearEnd($study_year) {
    list($start_year, $end_year) = explode('-', $study_year);
    return new DateTime($end_year . '-08-31');
}

function getWeekNumber($date, $study_year) {
    $study_start = getStudyYearStart($study_year);
    $target_date = new DateTime($date);
    
    // Разница в днях от начала учебного года
    $diff = $study_start->diff($target_date);
    ...
    // Номер недели (начинаем с 1)
    return floor($days_diff / 7) + 1;
}

function getWeekDates($study_year, $week) {
    $dates = [];
    $study_start = getStudyYearStart($study_year);
    
    // Первый день недели (понедельник)
    $first_day = clone $study_start;
    $first_day->modify('+' . (($week - 1) * 7) . ' days');
    
    // Корректируем на понедельник если нужно
    $day_of_week = (int)$first_day->format('N');
    if ($day_of_week != 1) {
        $first_day->modify('-' . ($day_of_week - 1) . ' days');
    }
...
function getAvailableStudyYears() {
    $current_year = (int)date('Y');
    $years = [];
    
    // Генерируем 5 лет назад и 5 лет вперед
    for ($i = -5; $i <= 5; $i++) {
        $year = $current_year + $i;
...
// Текущий учебный год и неделя
$current_study_year = isset($_GET['study_year']) ? $_GET['study_year'] : getStudyYear();
$current_week = isset($_GET['week']) ? (int)$_GET['week'] : getWeekNumber(date('Y-m-d'), $current_study_year);
$selected_faculty = isset($_GET['faculty_id']) ? $_GET['faculty_id'] : null;
$view_type = isset($_GET['view_type']) ? $_GET['view_type'] : 'faculty';
$selected_teacher = isset($_GET['teacher_id']) ? $_GET['teacher_id'] : null;
$selected_room = isset($_GET['room_id']) ? $_GET['room_id'] : null;

// Валидация номера недели
$max_weeks = 52; // Максимальное количество недель в учебном году
if ($current_week < 1) $current_week = 1;
if ($current_week > $max_weeks) $current_week = $max_weeks;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['room'])) {
    ...
    if (!empty($room)) {
        ...
        if ($stmt->execute()) {
            $_SESSION['success'] = "Кабинет $room успешно добавлена!";
        } else {
            $_SESSION['error'] = "Ошибка при добавлении группы: " . $conn->error;
        }
        $stmt->close();
...
$week_dates = getWeekDates($current_study_year, $current_week);
$available_study_years = getAvailableStudyYears();

// Получаем расписание в зависимости от типа просмотра
$schedule_data = [];
if ($view_type == 'faculty' && $selected_faculty) {
...
} elseif ($view_type == 'teacher' && $selected_teacher) {
    foreach ($week_dates as $date) {
        $stmt = $conn->prepare("
            SELECT s.*, u.fullname as teacher_name, f.name as faculty_name 
            FROM schedule s 
            ...
            ORDER BY s.lesson_number, s.faculty_id
        ");
        $stmt->bind_param('si', $date, $selected_teacher);
        $stmt->execute();
        $result = $stmt->get_result();
        $schedule_data[$date] = $result->fetch_all(MYSQLI_ASSOC);
        $stmt->close();
    }
} elseif ($view_type == 'room' && $selected_room) {
    foreach ($week_dates as $date) {
        $stmt = $conn->prepare("
            SELECT s.*, u.fullname as teacher_name, f.name as faculty_name 
            FROM schedule s 
            LEFT JOIN users u ON s.teacher_id = u.id 
            LEFT JOIN faculty f ON s.faculty_id = f.id 
            WHERE s.date = ? AND s.classroom = ? 
            ORDER BY s.lesson_number, s.faculty_id
        ");
        $stmt->bind_param('ss', $date, $selected_room);
        $stmt->execute();
        $result = $stmt->get_result();
        $schedule_data[$date] = $result->fetch_all(MYSQLI_ASSOC);
        $stmt->close();
...
<head>
    ...
    <style>
        ...
        .day-header { color: black; padding: 15px; text-align: center; font-weight: 600; min-width: 150px; }
        ...
        .today {color: #3498db;}
        .view-selector { background: #f8f9fa; border-radius: 8px; padding: 15px; margin-bottom: 20px; }
        .study-year-selector { background: #e3f2fd; border-radius: 8px; padding: 10px; margin-bottom: 15px; }
    </style>
</head>
<body>
    ...
    <div class="container mt-4">
        ...
        <ul class="nav nav-tabs mb-4" id="adminTabs" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="rooms-tab" data-bs-toggle="tab" data-bs-target="#room" type="button" role="tab">
        ...
        <div class="tab-content" id="adminTabsContent">
            ...
            <div class="tab-pane fade show active" id="schedule" role="tabpanel">
                <!-- Выбор учебного года -->
                <div class="card study-year-selector mb-4">
                    <div class="card-body py-2">
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Учебный год:</label>
                                <form method="GET" class="d-inline">
                                    <select class="form-select d-inline-block w-auto" name="study_year" onchange="this.form.submit()">
                                        <?php foreach($available_study_years as $study_year): ?>
                                            <option value="<?= $study_year ?>" <?= $current_study_year == $study_year ? 'selected' : '' ?>>
                                    ...
                                    <?php if ($view_type == 'faculty' && $selected_faculty): ?>
                                        <input type="hidden" name="faculty_id" value="<?= $selected_faculty ?>">
                                    <?php elseif ($view_type == 'teacher' && $selected_teacher): ?>
                                        <input type="hidden" name="teacher_id" value="<?= $selected_teacher ?>">
                                    <?php elseif ($view_type == 'room' && $selected_room): ?>
                ...
                <!-- Выбор типа просмотра и навигация -->
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="row align-items-center">
                            <div class="col-md-4">
                                <label class="form-label">Тип просмотра:</label>
                                <form method="GET" class="d-flex gap-2">
                                    <select class="form-select" name="view_type" onchange="this.form.submit()">
                                        <option value="faculty" <?= $view_type == 'faculty' ? 'selected' : '' ?>>По группам</option>
                                        <option value="teacher" <?= $view_type == 'teacher' ? 'selected' : '' ?>>По преподавателям</option>
                                        <option value="room" <?= $view_type == 'room' ? 'selected' : '' ?>>По кабинетам</option>
                                    </select>
                                    <input type="hidden" name="study_year" value="<?= $current_study_year ?>">
                                    <input type="hidden" name="week" value="<?= $current_week ?>">
                                </form>
                            </div>
                            ...
                            <div class="col-md-4">
                                <?php if ($view_type == 'faculty'): ?>
                                    ...
                                    <form method="GET" class="d-flex">
                                        ...
                                        <input type="hidden" name="view_type" value="faculty">
                                        <input type="hidden" name="study_year" value="<?= $current_study_year ?>">
                                        <input type="hidden" name="week" value="<?= $current_week ?>">
                                    </form>
                                <?php elseif ($view_type == 'teacher'): ?>
                                    ...
                                    <form method="GET" class="d-flex">
                                        ...
                                        <input type="hidden" name="view_type" value="teacher">
                                        <input type="hidden" name="study_year" value="<?= $current_study_year ?>">
                                        <input type="hidden" name="week" value="<?= $current_week ?>">
                                    </form>
                                <?php elseif ($view_type == 'room'): ?>
                                    <label class="form-label">Выберите кабинет:</label>
                                    <form method="GET" class="d-flex">
                                        <select class="form-select" name="room_id" onchange="this.form.submit()">
                                            ...
                                            while($room = $rooms->fetch_assoc()): ?>
                                                <option value="<?= $room['room'] ?>" <?= $selected_room == $room['room'] ? 'selected' : '' ?>>
                                                    Кабинет <?= $room['room'] ?>
                                                </option>
                                            <?php endwhile; ?>
                                        </select>
                                        <input type="hidden" name="view_type" value="room">
                                        <input type="hidden" name="study_year" value="<?= $current_study_year ?>">
                                        <input type="hidden" name="week" value="<?= $current_week ?>">
                                    </form>
                                <?php endif; ?>
                            ...
                            <div class="col-md-4 text-end">
                                <div class="btn-group">
                                    <?php
                                    $prev_url = "?study_year=$current_study_year&week=".($current_week-1)."&view_type=$view_type";
                                    $next_url = "?study_year=$current_study_year&week=".($current_week+1)."&view_type=$view_type";
                                    
                                    if ($view_type == 'faculty' && $selected_faculty) {
                                        $prev_url .= "&faculty_id=$selected_faculty";
                                        $next_url .= "&faculty_id=$selected_faculty";
                                    } elseif ($view_type == 'teacher' && $selected_teacher) {
                                        $prev_url .= "&teacher_id=$selected_teacher";
                                        $next_url .= "&teacher_id=$selected_teacher";
                                    } elseif ($view_type == 'room' && $selected_room) {
                                        $prev_url .= "&room_id=$selected_room";
                                        $next_url .= "&room_id=$selected_room";
                                    }
                                    ?>
                ...
                <?php if (($view_type == 'faculty' && $selected_faculty) || ($view_type == 'teacher' && $selected_teacher) || ($view_type == 'room' && $selected_room)): ?>
                <!-- Расписание -->
                ...
                <?php else: ?>
                    <div class="alert alert-info text-center">
                        <i class="bi bi-info-circle"></i> 
                        <?php if ($view_type == 'faculty'): ?>
                            Выберите группу для просмотра и редактирования расписания
                        <?php elseif ($view_type == 'teacher'): ?>
                            Выберите преподавателя для просмотра расписания
                        <?php elseif ($view_type == 'room'): ?>
                            Выберите кабинет для просмотра расписания
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <!-- Вкладка управления кабинетами -->
            <div class="tab-pane fade" id="room" role="tabpanel">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="bi bi-plus-circle"></i> Добавить новый кабинет</h5>
                    </div>
                    <div class="card-body">
                        <form method="POST" class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label">Название кабинета</label>
                                <input type="text" class="form-control" name="room" placeholder="Например: 101 или Актовый зал" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">&nbsp;</label>
                                <button type="submit" class="btn btn-primary d-block">Добавить кабинет</button>
                            </div>
                        </form>
                    </div>
                </div>

                <!-- Список существующих кабинетов -->
                <div class="card mt-4">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="bi bi-list-ul"></i> Существующие кабинеты</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php 
                                    $all_rooms = $conn->query("SELECT * FROM room ORDER BY room");
                                    while($room = $all_rooms->fetch_assoc()): ?>
                                        <tr>
                                            <td><strong>Кабинет <?= $room['room'] ?></strong></td>
                                        </tr>
                                    <?php endwhile; ?>
                                </tbody>
                            </table>
                        ...
                        <div id="normalForm">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="classroom">
                                    ...
                                    <?php 
                                    $rooms->data_seek(0);
                                    while($room_item = $rooms->fetch_assoc()): ?>
                                        <option value="<?= $room_item['room'] ?>"><?= $room_item['room'] ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                        </div>
    ...
    <script>
        ...
        // Запрет редактирования для просмотра по преподавателям и кабинетам
        document.addEventListener('DOMContentLoaded', function() {
            const viewType = '<?= $view_type ?>';
            if (viewType !== 'faculty') {
                const lessonCards = document.querySelectorAll('.lesson-card, .empty-lesson');
